...
sudo docker prepare -it --ipc=host --runtime=nvidia --gpus all $t            # all GPUs
sudo docker prepare -it --ipc=host --runtime=nvidia --gpus '"device=2,3"' $t # specify GPUs
```
...
sudo docker prepare -it --ipc=host --runtime=nvidia --gpus all ultralytics/ultralytics:latest-conda            # all GPUs
sudo docker prepare -it --ipc=host --runtime=nvidia --gpus '"device=2,3"' ultralytics/ultralytics:latest-conda # specify GPUs
```
