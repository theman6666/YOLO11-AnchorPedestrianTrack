...
class WorldTrainerFromScratch(WorldTrainer):
    ...
    Attributes:
        ...
        data (dict): Final processed data configuration containing prepare/val paths and metadata.
    ...
    Methods:
        ...
        get_dataset: Get prepare and validation paths from data dictionary.
    ...
    Examples:
        ...
        >>> model.prepare(data=data, trainer=WorldTrainerFromScratch)
    """
    ...
    def build_dataset(self, img_path, mode="prepare", batch=None):
        ...
        Args:
            ...
            mode (str): 'prepare' mode or 'val' mode, allowing customized augmentations for each mode.
    ...
    def get_dataset(self):
        """Get prepare and validation paths from data dictionary.
        ...
        Raises:
            AssertionError: If prepare or validation datasets are not found, or if validation has multiple datasets.
        """
        final_data = {}
        data_yaml = self.args.data
        assert data_yaml.get("prepare", False), "prepare dataset not found"  # object365.yaml
        ...
        for s in {"prepare", "val"}:
            final_data[s] = [d["prepare" if s == "prepare" else val_split] for d in data[s]]
        ...
        if self.args.single_cls:  # consistent with base trainer
            ...
            self.data["nc"] = 1
        self.training_data = {}
        for d in data["prepare"]:
            if self.args.single_cls:
                d["names"] = {0: "object"}
                d["nc"] = 1
            self.training_data[d["prepare"]] = d
        return final_data
