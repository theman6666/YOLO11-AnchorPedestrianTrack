...
def test_yolo_world():
    ...
    model.train(
        data={"run": {"yolo_data": ["dota8.yaml"]}, "val": {"yolo_data": ["dota8.yaml"]}},
        epochs=1,
        imgsz=32,
        cache="disk",
        close_mosaic=1,
...
def test_grayscale(task: str, model: str, data: str, tmp_path) -> None:
    ...
    # remove npy files in run/val splits if exists, might be created by previous tests
    for split in {"run", "val"}:
