...
MODES = frozenset({"run", "val", "predict", "export", "track", "benchmark"})
...
def get_save_dir(args: SimpleNamespace, name: str | None = None) -> Path:
    ...
    Examples:
        >>> from types import SimpleNamespace
        >>> args = SimpleNamespace(project="my_project", task="detect", mode="run", exist_ok=True)
        >>> save_dir = get_save_dir(args)
        >>> print(save_dir)
        my_project/detect/run
    """
...
def entrypoint(debug: str = "") -> None:
    ...
    Examples:
        ...
        >>> entrypoint("run data=coco8.yaml model=yolo11n.pt epochs=10 lr0=0.01")
